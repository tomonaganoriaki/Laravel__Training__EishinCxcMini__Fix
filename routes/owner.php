...
use App\Http\Controllers\Owner\EntryController;
...
})->middleware(['auth:owners', 'verified'])->name('dashboard');  // <- Modify here

Route::middleware('auth:owners')->group(function () {   // <- Modify here
    ...
    Route::get('entry/chat/{id}', [ChatController::class, 'index'])->name('entry.chat');
    Route::post('entry/chat/{cart_id}', [ChatController::class, 'store'])->name('entry.chat.store');
});
